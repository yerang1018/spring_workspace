...
public class MemberController {
	...
	public String login(MemberDTO dto, String url, HttpSession session) {	//	session 은 service로 넘어가지 않기 때문에
		MemberDTO login = memberService.login(dto);				//	파라미터를 하나로 묶어서 보내지 않아도 된다 
		session.setAttribute("login", login);
		if(url == null) {	
			url = "/";
		}							//	write 페이지에서 로그인이 안되어 있으면 로그인페이지로 보내짐 	
		return "redirect:" + url;	//	로그인 페이지에서 로그인을 하고 넘기면   return "redirect:" + url  에 의해서 write 페이지로 바로 넘어온다 !
	}
